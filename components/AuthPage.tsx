...
const AuthPage = () => {
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    try {
      ...
      setApiError(error.message || `${provider} login failed`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen pt-20 bg-black text-white flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-black border border-white/20 rounded-lg overflow-hidden p-6">
          <h2 className="text-2xl font-bold text-center mb-6">
            Welcome to Pollar
          </h2>

          {apiError && (
            <div className="p-3 bg-red-900/50 border border-red-500 rounded-md text-sm mb-4">
              {apiError}
            </div>
          )}
          ...
          <div className="space-y-4">
            <button
              ...
              disabled={isLoading}
              className="flex cursor-pointer items-center justify-center gap-3 w-full py-2 px-4 bg-black border border-white/20 rounded-md hover:bg-white/10 transition-colors disabled:opacity-50"
            >
              {isLoading ? (
